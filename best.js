...
try {
    ...
    const validWords = data.map(line => line.toLowerCase())
        .filter(word => {
            let isWordValid = true

            word.split('').forEach((letter, index) => {
                if (greens[index] !== undefined && greens[index] !== letter) isWordValid = false;
            })
    ...
    validWords.forEach(word => {
        ...
        word.split('').forEach((letter, i) => {
            ...
            letterPosFrequency[i][letter] = (letterPosFrequency[i][letter] || 0) + 1
        })
    ...
    const calculateWordScore = (word) => {
        ...
        return Object.keys(wordLetterScore).reduce((a, b) => a + wordLetterScore[b], 0) + posLetterScore
    }
